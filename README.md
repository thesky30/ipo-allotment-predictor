...
> **当前状态（2026-06-01）**：数据清洗、三阶段（T-6 / T-1 / T+1）建模、扩张窗口时间序列回测与 Streamlit 网页演示均已完成并开箱即用。正式生产口径为 **T-6 LightGBM（询价前）**，OOS Spearman ≈ 0.62。未入库新股冷启动、发行安排公告 PDF 回填、招股书财务/可比公司名单抽取、Tushare 行业 PE / peer PE 回填、手动表单分组和字段来源展示均已落地。详见 [技术架构与模型结果](#技术架构与模型结果)。
