...
> **当前状态（2026-06-01）**：数据清洗、三阶段（T-6 / T-1 / T+1）建模、扩张窗口时间序列回测与 Streamlit 网页演示均已完成并开箱即用。正式生产口径为 **T-6 LightGBM（询价前）**，OOS Spearman ≈ 0.62。未入库新股冷启动、发行安排公告 PDF 回填、招股书财务/申万行业/可比公司名单抽取、Tushare 行业 PE / peer PE 回填、手动表单分组和字段来源展示均已落地。详见 [技术架构与模型结果](#技术架构与模型结果)。
...
- ✅ 页面会展示已回填字段及来源，并常显已识别的申万行业、3 年营收 CAGR、可比公司名单摘要；行业 PE 从本地申万行业行情缓存回填到 `industry_pe_at_ipo`，避免页面触发 Tushare `sw_daily` 频率限制；可比公司 PE 优先由招股书可比公司名单 + Tushare `daily_basic.pe_ttm` 计算后回填到 `comparable_pe_avg_ex_nonrecurring`。只有未识别到招股书可比公司名单时，才显示“申万行业成分股 PE”兜底入口，并明确它不是行业 PE。
...
- [x] 招股书抽取：分类定位相关页，抽取财务字段、申万一级行业和可比公司名单；可比公司锚点页会携带后一页，减少表格跨页漏读。
- [x] Tushare 估值接入：行业 PE 由本地缓存回填，只回填行业 PE；可比公司名单解析后计算 peer PE；可比公司名单缺失时才支持申万行业成分股 PE 作为可比公司 PE 兜底参考。
...
| 估值 | `comparable_pe_avg_ex_nonrecurring` | 招股书可比公司名单 + Tushare peer PE | 可比公司估值影响询价前吸引力判断 |
...
| 行业行情滚动因子 | 已有申万代码-名称/Tushare 指数映射，但尚未验证严格向前滚动口径并纳入正式模型 |
...
# 7.（可选）Tushare 数据刷新：市场/申万行业行情与行业 PE
python scripts/market_source.py --start-date 20190101

# 8.（可选）同行业 peer PE 近似统计
python scripts/peer_valuation.py --sw-level1-code 1000042211000000 --trade-date 20260529

# 9.（可选）按招股书可比公司名称计算 peer PE
...
scripts/import_sw_pe_excel.py            Wind 导出申万一级 PE TTM Excel → 本地 CSV + SQLite 缓存
scripts/peer_valuation.py                Tushare 可比公司/同行业成分 + daily_basic → peer PE 统计
scripts/reference_data.py                历史参考数据加载 + 申万一级行业代码/名称归一化
...
> 后续待办项（页面 UI 优化、Tushare 刷新运维化、策略层评估等）详见 `需要完善的部分.md`。
...
Streamlit Cloud 的 Secrets 建议配置：

```toml
LLM_API_KEY = "..."
LLM_BASE_URL = "..."   # OpenAI 兼容供应商可填；OpenAI 官方可不填
LLM_MODEL = "..."      # 不填默认 gpt-4o-mini
TUSHARE_TOKEN = "..."  # 用于行业 PE / peer PE 自动回填；缺失时仍可手动输入
```
