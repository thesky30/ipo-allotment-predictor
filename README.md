...
- ✅ 页面会展示已回填字段及来源，并常显已识别的申万行业、3 年营收 CAGR、可比公司名单摘要；行业 PE 可从 Tushare 申万行业行情缓存或实时拉取后回填到 `industry_pe_at_ipo`；可比公司 PE 优先由招股书可比公司名单 + Tushare `daily_basic.pe_ttm` 计算后回填到 `comparable_pe_avg_ex_nonrecurring`。只有未识别到招股书可比公司名单时，才显示“申万行业成分股 PE”兜底入口，并明确它不是行业 PE。
...
- [x] Tushare 估值接入：行业 PE 可用缓存或实时拉取，只回填行业 PE；可比公司名单解析后计算 peer PE；可比公司名单缺失时才支持申万行业成分股 PE 作为可比公司 PE 兜底参考。
