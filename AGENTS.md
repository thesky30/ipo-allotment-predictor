...
scripts/import_sw_pe_excel.py      Wind 申万一级 PE TTM Excel → processed CSV + SQLite 缓存
...
- [x] 网页界面优化第一轮：手动表单已按“基础信息 / 发行结构 / 申购规则 / 财务估值”分组；PDF 抽取后展示字段来源和已识别摘要；页面从本地行业 PE 缓存回填行业 PE，避免 `sw_daily` 频率限制；可比公司 PE 优先按招股书名单拉取，未识别名单时才允许用手动选择的申万行业成分股 PE 兜底。
