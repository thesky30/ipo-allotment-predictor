...
scripts/peer_valuation.py          Tushare 同行业成分 + daily_basic → peer PE 近似统计
...
- [x] 数据刷新基础层：`scripts/market_source.py` 已可通过 Tushare 刷新全市场成交/收益、申万一级行业代码-名称映射、申万行业行情与 PE，并落盘到 processed CSV / SQLite。AkShare 备源仍待后续补。
- [x] 同行业估值近似层：`scripts/peer_valuation.py` 已可用 Tushare 申万行业成分 + `daily_basic.pe_ttm` 计算同行业 peer PE 均值/中位数；该值是市场近似参考，不覆盖招股书披露口径的 `comparable_pe_avg_ex_nonrecurring`。
...
- **数据持续刷新已落地 Tushare 基础层**。Wind 无持续 API 权限是硬约束：训练用 Wind 一次性历史导出（质量最高）；生产/持续刷新改用可编程开放源。当前 `scripts/market_source.py` 提供 Tushare 主路径；AkShare 备源仍待后续补。未来若拿到 Wind 量化接口（WindPy），可在同一抽象层替换数据源。
...
- `scripts/peer_valuation.py`：按申万一级行业成分和 `daily_basic.pe_ttm` 计算同行业 peer PE 均值/中位数，供招股书未披露可比 PE 时展示为近似参考。
- 边界：Tushare peer PE 是市场数据近似口径，不等同于招股书披露的“可比公司扣非后市盈率平均值”，不得直接覆盖正式模型字段 `comparable_pe_avg_ex_nonrecurring`。
- 运行示例：
